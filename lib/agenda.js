...
var Agenda = module.exports = function(config) {
  ...
  this._limitJobsPerInterval = config.limitJobsPerInterval || null;
  this._definitions = {};
  this._runningJobs = [];
  this._jobQueue = [];
...
Agenda.prototype.database = function(url, collection, options) {
  ...
  this._db.ensureIndex("nextRunAt", ignoreErrors)
          ...
          .ensureIndex( {nextRunAt: 1, priority: -1}, ignoreErrors);
...
/**
 * Find and lock the next random job up for processing
 * @param {Function} cb
 * @protected
 */
Agenda.prototype._findAndLockFirstUpJob = function(cb) {
  var now = new Date(),
      lockDeadline = new Date(Date.now().valueOf() - this._defaultLockLifetime);

  this._db.findAndModify(
    {
      nextRunAt: {$lte: this._nextScanAt},
      lockedAt: { $not : { $gt: lockDeadline } },
      disabled: { $ne: true }
    },
...
function processJobs(extraJob) {
  ...
  if (self._limitJobsPerInterval) {
    maxJobs = self._limitJobsPerInterval - self._runningJobs.length;
  }
  ...
  function jobQueueFilling(name) {
    ...
    if (maxJobs) {
      ...
      self._findAndLockFirstUpJob(findJobCb);
    } else {
      self._findAndLockNextJob(name, definitions[name], findJobCb);
    }
  }
